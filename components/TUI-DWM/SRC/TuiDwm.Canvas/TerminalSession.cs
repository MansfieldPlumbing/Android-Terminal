...
// One ConPTY session → VT parser → CellBuffer → SKBitmap blit.
// Completely self-contained. No plugin system. AOT-safe.
//
// The pump thread does: pty.OutputStream.Read → ProcessBytes → update CellBuffer → render to BlitSurface.
// Callers read BlitSurface directly.
public sealed class TerminalSession : IDisposable
{
    ...
    public SKBitmap BlitSurface { get; private set; }
    ...
    private void ResizeBlit(int cols, int rows)
    {
        ...
        lock (BufferLock)
        {
            // "Megatexture" approach: Allocate a fixed bounds that covers any realistic terminal size.
            // 4096x4096 costs ~64MB per session and easily allows ~500+ cols and ~240+ rows.
            // We allocate it once and draw to a sub-region. Terminal resizes become purely logical.
    ...
    public void RenderBlitIfNeeded()
    {
        ...
        lock (BufferLock)
        {
            // Only clear the actively used region to save memory bandwidth on padded surfaces
            _canvas.DrawRect(0, 0, cellW * FrontBuffer.Width, cellH * FrontBuffer.Height, new SKPaint { Color = SKColors.Transparent, BlendMode = SKBlendMode.Src });
            ...
            for (int row = 0; row < FrontBuffer.Height; row++)
            {
                ...
                for (int col = 0; col < FrontBuffer.Width; col++)
                {
                    ...
                    if (ch > ' ')
                    {
                        ...
                        _canvas.DrawText(ch.ToString(), px, py + cellH - 3f, _font, _cellFg);
                    }
                }
            }
        }
    ...
    {
        _pty = PtySession.Start(shell, (short)_back.Width, (short)_back.Height);
        _running = true;
        _pumpThread = new Thread(PumpLoop)
        {
            IsBackground = true,
            Name         = $"TuiCanvas-PTY-{Title}"
        };
        _pumpThread.Start();
    }
    ...
    public void SendInput(string text)
    {
        ...
        try { _pty.InputStream.Write(bytes, 0, bytes.Length); _pty.InputStream.Flush(); }
        catch { }
    }
    ...
    // ── Minimal VT parser ─────────────────────────────────────────────────
    // Handles: printable chars, CR/LF/BS/TAB, CSI cursor/erase/SGR, alt screen.
    ...
    private void ApplySgr(string[] parts)
    {
        for (int i = 0; i < parts.Length; i++)
        {
            ...
            switch (code)
            {
                ...
                case 34: case 35: case 36: case 37: _fg = (byte)(code - 30); break;
                case 39: _fg = 7; break;
                case 40: case 41: case 42: case 43:
                case 44: case 45: case 46: case 47: _bg = (byte)(code - 40); break;
                case 49: _bg = 16; break;
                case 90: case 91: case 92: case 93:
                case 94: case 95: case 96: case 97: _fg = (byte)(code - 90 + 8); break;
                case 100: case 101: case 102: case 103:
                case 104: case 105: case 106: case 107: _bg = (byte)(code - 100 + 8); break;
    ...
    private void EnterAltScreen()
    {
        ...
        // Swap: current content saved, alt screen blank
        var tmp = _back;
        _back = _altScreen;
        _altScreen = tmp;
        _useAlt = true;
