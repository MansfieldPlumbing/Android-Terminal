// PtySession.cs — Win32 ConPTY RAII wrapper
//
// Owns the PTY, the child process, and a Job Object dead man's switch.
// JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE: the child cannot outlive this process
// for any reason — clean exit, crash, OOM, Task Manager kill.
//
// CREATE_NO_WINDOW is intentionally NOT passed to CreateProcess.
// It prevents pwsh from attaching to the pseudo-console.
// EXTENDED_STARTUPINFO_PRESENT alone is correct.
...
{
    private nint           _hPC;
    private SafeFileHandle _inRead  = null!;
    private SafeFileHandle _inWrite = null!;
    private SafeFileHandle _outRead = null!;
    private SafeFileHandle _outWrite = null!;
    ...
    public static PtySession Start(string commandLine, short cols, short rows,
    ...
    private void StartCore(string commandLine, short cols, short rows, string? workingDirectory)
    {
        ...
        int hr = NativeMethods.CreatePseudoConsole(
            new COORD { X = cols, Y = rows }, _inRead, _outWrite, 0, out _hPC);
        if (hr != 0)
        ...
        nint cmdPtr = Marshal.StringToCoTaskMemUni(commandLine);
        nint dirPtr = workingDirectory is null ? nint.Zero : Marshal.StringToCoTaskMemUni(workingDirectory);
        ...
        try
        {
            ...
            if (!NativeMethods.CreateProcess(
                    nint.Zero, cmdPtr, nint.Zero, nint.Zero, false,
                    (Win32.EXTENDED_STARTUPINFO_PRESENT | 0x08000000),
                    nint.Zero, dirPtr, ref si, out var pi))
            ...
            // Close PTY-side pipe ends after CreateProcess.
            // ConPTY does not duplicate these handles — closing before CreateProcess
            // would leave ConPTY with a dead write end and block ReadFile forever.
            _inRead.Dispose();
            _outWrite.Dispose();
        }
        finally
        ...
        // Arm dead man's switch — child cannot outlive this process
        ...
        NativeMethods.SetInformationJobObject(
            _hJob, Win32.JobObjectExtendedLimitInformation,
            ref limits, Marshal.SizeOf<JOBOBJECT_EXTENDED_LIMIT_INFORMATION>());
        NativeMethods.AssignProcessToJobObject(_hJob, _hProcess);
    ...
    public void Dispose()
    {
        ...
        _hJob?.Dispose(); // Job Object closes → kernel kills child if still running
...
internal static partial class NativeMethods
{
    ...
    internal static partial bool SetInformationJobObject(
        SafeFileHandle hJob, int JobObjectInformationClass,
